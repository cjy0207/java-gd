...
//	fromIndex: 검색 시작 위치 지정
	...
	public static void main(String[] args) {
		...
		System.out.println(count(original, search));
		
	}
	
	public static int count(String original, String search) {
		int count = 0;
		int fromIndex = 0;
		
		while(true) {
			fromIndex = original.indexOf(search,fromIndex);
			if(fromIndex == -1) break;
			
			count++;
			fromIndex = fromIndex + search.length();
		}
		return count;
	}
