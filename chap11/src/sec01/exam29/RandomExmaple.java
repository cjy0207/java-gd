...
public class RandomExmaple {
	...
	public static void main(String[] args) {
		...
		//상수, 실수, 논리값 등 다양한 타입의 난수 생성 가능
		//반복, 시드 필요, 테스트 용도 또는 멀티 스레드 환경에서 사용
		
		Random random = new Random();
		
		//seed 시드란?
		//난수 생성기의 초기값
		//일한시드를 넣으면 항상 같은 난수 순서(시퀸스)가 생김
		//시드를 지정하지 않으면 시스템의 현재 시각에 따라 자동으로 지정
		random.setSeed(1234);
		...
		//범위 지정(이상, 미만)
		int randInt4 = random.nextInt(0, 10); //0이상 10미만
		...
		double randDouble3 = random.nextDouble(1.14, 5.67);
		double randDouble4 = random.nextDouble(1.14, 5.67);
