...
public class HashSetExample {
	...
	public static void main(String[] args) {
		...
		Set<String> set = new HashSet<>(); // 생략하면 왼쪽 Set에 지정된 타입에 따라 감
		
		//객체 추가
		//추가 할 때마다 중복 확인(hashCode와 equals 호출) 후 추가 함
		//기존에 같은 객체가 없을 떄만 저장하고 true반환
		set.add("Java");
		set.add("JDBC");
		set.add("Servlet/JSP");
		...
		// Iterable Iterator - 컬렉션 조회 방법의 표준화(=인터페이스
		// 여러 종류의 컬렉션(Iterable)을 같은 방법(Iterator)으로 읽을 수 있음
		
		//Iterator(반복자)로 모든 요소를 하나씩 가져오기
		//컬렉션 요소를 하나씩 순회 할 수 있도록 도와주는 인터페이스
		//타입 파라미터는 저장된 객체와 동일하게 작성
		Iterator<String> iterator = set.iterator(); //iterator 구형 객체를 반환, 제네릭 생략은 오브젝트가 됨
		...
		//객체 삭제
		//index를 갖고 있는 List와 다르게 index가 없음
		//객체의 hashCode와 equals를 기준으로 판단
		set.remove("JDBC");
		set.remove("iBATIS");
		...
		if(!set.isEmpty()) { //컬렉션이 비어있는지 조사
			//주어진 객체가 저장되어 있는지 조사
			System.out.println(set.contains("Servlet/JSP"));
		}
		System.out.println();
		...
		//집합과 관련된 메소드
		//Collection에 변화가 있으면 true, 아니면 false
		
		//합집합
		//두개의 HashSet을 하나로 합쳐줌
		//이때 두개의 HashSet에 공통적으로 포함되어 있는 데이터는 제거
//		System.out.println(class1.addAll(class2));
//		System.out.println("합집합 : " + class1);
		
		//교집합
		//두개의 HashSet에서 공통된 요소만 추려줌
...
//		System.out.println("차집합 : " + class1);
		...
		//정렬을 하려면 다른 컬렉션으로 변환 변환 후 정렬
		//List로 변화 후 Collectionsort or List.sort이용
...
//		fruitsList.sort(); //기본 정렬 기준이 아니라서 Comparator로 따로 정렬 기준을 만들어줘야함
