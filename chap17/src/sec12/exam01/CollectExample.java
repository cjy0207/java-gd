...
public class CollectExample {
	...
	public static void main(String[] args) {
		...
		// 스트림을 컬렉션으로 변롼 : toList(), toSet(), toMap() 등
		// 남학생만 묶어서 List생성
//		List<Student> maleList = totalList.stream()
//				.filter(s -> "남".equals(s.getGender()))
//				.collect(Collectors.toList()); //요소를 리스트에 담아서(toList) 모으겟다(collect

		// 자바 16부터는 좀 더 편리하게 요소 스트림에서 List 컬렉션을 얻을 수 있음
		List<Student> maleList = totalList.stream().filter(s -> "남".equals(s.getGender())).toList();

		maleList.stream().forEach(s -> System.out.println(s.getName()));

		// 학생 이름을 키로, 학생의 점수를 값으로 갖는 Map 생성
		totalList.stream().collect(Collectors.toMap(
